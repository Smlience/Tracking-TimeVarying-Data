#include "VolumeLightingRender.h"
...
void VolumeLightingRender::initTextures()
{
	...
	//m_gradientTex = new Texture((GLubyte*)0, Vector3i(0,0,0), GL_RGB, GL_RGB32F, GL_FLOAT, Texture::LINEAR, false);
	m_gradientTex->setWrapping(Texture::CLAMP);
}

void VolumeLightingRender::paintGL()
{
	if (!m_volume.loaded())
	{
...
void VolumeLightingRender::initShader()
{
...
void VolumeLightingRender::setKa( double v )
{
	m_ka = v;
	update();
}

void VolumeLightingRender::setKs( double v )
{
	m_ks = v;
	update();
}

void VolumeLightingRender::setKd( double v )
{
	m_kd = v;
	update();
}

void VolumeLightingRender::setShininess( double v )
{
	m_shininess = v;
	update();
}

void VolumeLightingRender::setLightPos( Vector3d pos )
{
	m_lightPos.set(pos.x, pos.y, pos.z);
	update();
}

void VolumeLightingRender::onDataLoaded( const Volume* v )
{
	//计算梯度模
