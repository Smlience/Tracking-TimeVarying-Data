...
vxHistogramPainter::vxHistogramPainter(QWidget* parent, Vector2f xRange, Vector2f yRange, int leftSpace, int rightSpace,
...
vxHistogramPainter::~vxHistogramPainter()
{
//	delete m_pHistogram;
	delete m_pCache;
}

void vxHistogramPainter::setHistogram(IntensityHistogram* histogram) {
//	delete m_pHistogram;
...
void vxHistogramPainter::setXRange(const Vector2f& xRange) {
	if (xRange != m_xRange) {
		m_xRange = xRange;
		delete m_pCache;
		m_pCache = 0;
...
void vxHistogramPainter::updateHistogram() {
	delete m_pCache;
	m_pCache = 0;
}

void vxHistogramPainter::paintEvent(QPaintEvent* event) {
	event->accept();
...
Vector2f vxHistogramPainter::wtos(const Vector2f& p) const {
