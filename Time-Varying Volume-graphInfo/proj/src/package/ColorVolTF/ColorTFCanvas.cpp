...
ColorTFCanvas::ColorTFCanvas(QWidget* parent)
: QWidget(parent)
, m_iMaximumIntensity(255)
, m_bIsNoColor(false)
, m_xRange(0.f, 1.f), m_yRange(0.f, 1.f), m_xAxisText(tr("Color")), m_yAxisText(tr("opacity")), m_bShowTFCurve(true), m_bColorbarSortable(false)
{
	m_iPadding = 12;
	m_iBottomSpace = 42;
	m_iArrowLength = 10;
	...
	connect(m_colorBar, SIGNAL(mousePressedAt(int)), this, SLOT(mousePressedAt(int)));

	setObjectName("ColorTFCanvas");
...
ColorTFCanvas::~ColorTFCanvas()
{
	SAFE_DELETE(m_tf);
	SAFE_DELETE(m_keyColletion);
}

QSize ColorTFCanvas::minimumSizeHint() const
{
	return QSize(300,100);
}

QSize ColorTFCanvas::sizeHint() const
{
	return QSize(300, 100);
}

QSizePolicy ColorTFCanvas::sizePolicy() const
{
	return QSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
}

bool ColorTFCanvas::eventFilter( QObject *obj, QEvent *e )
{
...
void ColorTFCanvas::resizeEvent( QResizeEvent* event )
{
...
void ColorTFCanvas::paintEvent( QPaintEvent* event )
{
	if (!m_keyColletion)
		return;
	event->accept();
...
void ColorTFCanvas::mousePressEvent(QMouseEvent* event)
{
...
void ColorTFCanvas::mouseMoveEvent(QMouseEvent* event)
{
	event->accept();
...
void ColorTFCanvas::mouseReleaseEvent(QMouseEvent* event)
{
	event->accept();
	if (event->button() == Qt::LeftButton) {
		m_bIsDragging = false;
...
void ColorTFCanvas::keyPressEvent(QKeyEvent* event)
{
...
void ColorTFCanvas::keyReleaseEvent(QKeyEvent* event)
{
	unsetCursor();
...
// void ColorTFCanvas::mouseDoubleClickEvent(QMouseEvent *event) {
// 	event->accept();
// 	if (event->button() == Qt::LeftButton)
// 		changeCurrentColor();
// }
...
void ColorTFCanvas::setupTransfunc()
{
...
void ColorTFCanvas::updateCoordinatesTooltip( QPoint pos, Vector2f values )
{
...
int ColorTFCanvas::hitLine( const Vector2f& p )
{
	int hit = -1;
...
Vector2f ColorTFCanvas::wtos( Vector2f p )
{
...
Vector2f ColorTFCanvas::stow( Vector2f p )
{
...
void ColorTFCanvas::insertNewKey( Vector2f& hit )
{
	hit.x = max(hit.x,0.f);
	hit.x = min(hit.x,1.f);
	hit.y = max(hit.y,0.f);
...
TFMappingKey* ColorTFCanvas::getOtherKey( TFMappingKey* selectedKey, bool chooseLeftPart )
{
...
void ColorTFCanvas::paintKeys( QPainter& paint )
{
	for (int i=0; i<m_keyColletion->getNumKeys(); ++i) {
		TFMappingKey *key = m_keyColletion->getKey(i);
...
void ColorTFCanvas::drawMarker( QPainter& paint, const Color4& color, const Vector2f& p, int props /*= 0*/ )
{
	if (m_bIsNoColor)
...
QColor ColorTFCanvas::Col2QColor( const Color4& color )
{
	return QColor(color.r, color.g, color.b); // ignore alpha
}

Color4 ColorTFCanvas::QColor2Col( const QColor& color )
{
	return Color4(color.red(), color.green(), color.blue(), 255); // ignore alpha
}

Color4 ColorTFCanvas::getMappingKeyValue( float value ) const
{
	if (m_keys.empty())
...
void ColorTFCanvas::setVolume(const Volume* vol )
{
	resetTF();
	m_tf->setVolume(vol);
	m_pHistogramPainter->setHistogram(m_tf->getClusterHistogram());
	m_colorBar->setColor(m_tf->tfData(), m_tf->getClusterCount());
	emit sendClusterInfo(m_tf->clusters());
}

void ColorTFCanvas::mousePressedAt( int idx )
{
	if(m_bColorbarSortable)
	{
		int c_idx = m_colorBar->colorIndexAt(idx);
		int c_sidx = c_idx;
		int c_range = 1;
		// 根据选中颜色进行排序，并获取排序后的用户选中索引，及阈值内的索引区段
		m_tf->sortNearestTo(c_idx, c_idx, c_sidx, c_range);
		m_colorBar->setIndexMap(m_tf->getIndexMap());
		// 生成TF曲线
		this->genGaussTF(c_idx, c_sidx, c_range);
		// 设置ColorBar的选中
		m_colorBar->setSelected(c_sidx, c_range);
		update();
	}	
}

void ColorTFCanvas::genGaussTF( int cidx, int sidx, int range )
{

}

void ColorTFCanvas::showTFCurve( bool show )
{
	m_bShowTFCurve = show;
	update();
}

void ColorTFCanvas::saveTF()
{
	QString filename = QFileDialog::getSaveFileName(this, "Save File", QString(), "Color TF (*.ctf)");
	if (!filename.isNull())
	{
...
void ColorTFCanvas::loadTF()
{
	QString filename = QFileDialog::getOpenFileName(this, "Open File", QString(), "Color TF (*.ctf)");
	if (!filename.isNull())
	{
...
// 		key->setColor(pkeys[0].color);
// 		key->setIntensity(pkeys[0].intensity);
// 		key =	m_keyColletion->getKey(1);
// 		key->setColor(pkeys[1].color);
// 		key->setIntensity(pkeys[1].intensity);
// 		for (int i = 2; i < c; ++i)
// 		{
// 			key = new TFMappingKey(pkeys[i].intensity, pkeys[i].color);
// 			m_keyColletion->addKey(key);
// 		}
	}
}

void ColorTFCanvas::showHistogram( bool show )
{
