...
ScatterPlot::ScatterPlot( QWidget *parent )
	: QWidget(parent), m_xRange(0, 256), m_yRange(0, 256), m_partitions(8, 8), m_histogram(NULL), m_histoImg(NULL), m_imgType(ScatterPlot::Histogram)
{
...
QPointF ScatterPlot::transform(const QPointF& pos)
{
	float x = pos.x();
	float y = pos.y();
...
void ScatterPlot::setData( const unsigned char *v1, std::pair<int,int> v1Range, const unsigned char*v2, std::pair<int,int> v2Range, Vector3i dim )
{
	m_xRange = v1Range;
	m_yRange = v2Range;
...
void ScatterPlot::drawHistogram()
{
	...
	for(j = idx = 0; j < m_dim.y; ++j)
	{
		for (i = 0; i < m_dim.x; ++i, ++idx)
		{
			//idx = j * m_dim.x + i
			int opacity;
			if (m_imgType == ScatterPlot::Scatter) // Scatter plot, 全白或全黑
			{
...
QMargins ScatterPlot::margins() const
{
	return m_margins;
}

QSize ScatterPlot::plotSize() const
{
	return m_size;
}

QRect ScatterPlot::plotRect() const
{
	return QRect(m_margins.left(), m_margins.top(), m_size.width(), m_size.height());
}

Vector2i ScatterPlot::dataDimension() const
{
	return m_dim;
}
