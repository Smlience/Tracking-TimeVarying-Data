...
// scheme == 0 => RING
// scheme == 1 => NEST
void neighbors(long nside, int pix, int *result, int scheme)
{
	...
	int ix, iy, face_num;
	(scheme==0) ?
		ring2xyf(nside, pix,&ix,&iy,&face_num) : nest2xyf(nside, pix,&ix,&iy,&face_num);

	const int nsm1 = nside-1;
	if ((ix>0)&&(ix<nsm1)&&(iy>0)&&(iy<nsm1))
	{
		if (scheme==0)
			for (int m=0; m<8; ++m)
				result[m] = xyf2ring(nside, ix+xoffset[m],iy+yoffset[m],face_num);
		else
			for (int m=0; m<8; ++m)
				result[m] = xyf2nest(nside, ix+xoffset[m],iy+yoffset[m],face_num);
	}
	else
	{
		for (int i=0; i<8; ++i)
		{
			...
			if (f>=0)
			{
				if (swaparray[nbnum][face_num]&1) x=nside-x-1;
				if (swaparray[nbnum][face_num]&2) y=nside-y-1;
				if (swaparray[nbnum][face_num]&4) {
					int _Tmp = x;
					x = y;
					y = _Tmp;
				}
				result[i] = (scheme==0) ? xyf2ring(nside, x,y,f) : xyf2nest(nside, x,y,f);
			}
			else
				result[i] = -1;
		}
